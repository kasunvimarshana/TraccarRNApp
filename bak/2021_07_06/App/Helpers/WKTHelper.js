...
let coordFormat_circle = new RegExp('^' + numberRegexp.source);
  ...
  function multicoords () {
    ...
    while (elem =
      ...
      if (elem === '(') {
        ...
        Array.prototype.push.apply(pointer, elem.split(/\s/g).map(parseFloat).reverse());
      } else {
        return null;
      }
      white();
  ...
  function coords () {
    ...
    while (pt =
      ...
      if (pt === ',') {
        ...
        Array.prototype.push.apply(item, pt.split(/\s/g).map(parseFloat).reverse());
      }
      white();
    }
